...
class TrainsetLoader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        lr_dir = self.trainset_dir + '/' + self.video_list[idx_video] + '/lr_x' + str(self.upscale_factor)
        ...
        for id in range(0, num):
            lr = Image.open(lr_dir + '/' + str(idx_frame + id).rjust(4,'0') + '.png')
            lr = np.array(lr, dtype=np.float32) / 255.0
            lr_y = rgb2y(lr)
            LR.append(lr_y)
            hr = Image.open(hr_dir + '/' + str(idx_frame + id).rjust(4,'0') + '.png')
        ...
        # data augmentation
        LR_T, HR_T = augmentation()(LR_T, HR_T)
...
class TestsetLoader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        LR3 = Image.open(dir + '/lr_' + str(idx + 4).rjust(4,'0') + '.png')
        LR4 = Image.open(dir + '/lr_' + str(idx + 5).rjust(4,'0') + '.png')
    ...
    #         LR_new.append(LR_y)
    #     # print(len(LR_new[0]),len(LR_new[1]),len(LR_new[2]),len(LR_new[3]),len(LR_new[4]))
    #     if num == 3:
    #         LR_T = np.concatenate((LR_new[0], LR_new[1], LR_new[2]), axis=2)
    #     if num == 5:
    #         LR_T = np.concatenate((LR_new[0], LR_new[1], LR_new[2], LR_new[3], LR_new[4]), axis=2)
    ...
    def __len__(self):
        return self.frame_list.__len__() - 5
...
def random_crop(HR, LR, patch_size_lr, upscale_factor):
    ...
    # h_end_hr - h_start_hr = 128; w_end_hr - w_start_hr = 128
    # h_end_lr - h_start_lr = 128; w_end_lr - w_start_lr = 32
